...
- **Asset** — `category` (`equipo`/`accesorio`), `type` (laptop, escritorio, all_in_one, monitor, mouse, teclado, celular, tablet, cargadores, `disco_duro`/`adaptador` en Accesorios, `router`/`switch`/`camara_ip`/`nvr`/`poe_injector`/`ups`/`insumo_red` en Activos > Infraestructura, etc. — ver `ASSET_TYPE_LABELS`/`ASSET_GROUPS`/`ACCESSORY_GROUPS` en `frontend/src/config/assetFields.js`), `brand`, `model`, `serialNumber`, `inventoryTag`, `status` (`disponible`/`asignado`/`baja`), `purchaseDate`, `stockTotal`, `location`, `notes`, `specs` (Mixed — usar `markModified('specs')` + `.save()`, **no** `findByIdAndUpdate`).
  - **Ojo con agregar un tipo nuevo:** no basta con el enum del modelo y `assetFields.js` — hay listas de tipos independientes (para que no queden invisibles) en `Stock.jsx` (`STOCK_SECTIONS`), `Dashboard.jsx` (`CATEGORIES`), `Assignments.jsx` (`FILTER_CATS`), `Accessories.jsx` (`TABS`) y `Assets.jsx` (`TABS`), además de `importCategories.js` (`IMPORT_CATEGORIES`, que también define a qué `category` pertenece cada bucket de importación — sin eso, un activo importado se queda con el `category` default del esquema sin importar cuál se haya elegido).
...
| `platform-accounts-erp` | Mismos endpoints que `platform-accounts` (sin `unregistered-corporate`, no aplica) más `POST /bulk-import` (`{ platform, accounts: [{ employeeId, username }] }` → crea en lote sin contraseña, marcadas `passwordPending`; usado por el importador de Excel) y `GET /gmail-lookup?employeeId=` (correo+contraseña de las cuentas Gmail de ese empleado, para "¿Ya existe con Gmail?" al dar de alta — no requiere `canManageGmailAccounts`) — todo sobre la colección `PlatformAccountErp`, completamente aislada — requiere el permiso `canManagePlatformAccountsErp` |
...
- `platformAccounts.js` y `gmailAccounts.js` (`GET /:id/responsiva`, misma lógica en ambos) — "Solicitud y Carta Responsiva de Cuenta de Acceso a Plataformas Digitales", basada en la plantilla `Responsiva_Cuentas_Plataformas.docx` del usuario. Se llena con los datos del empleado y de la cuenta (nunca la contraseña), y usa `[ ]`/`[X]` en vez de ☐/☒ para los checkbox de plataforma, porque esos caracteres Unicode no se renderizan con la fuente estándar de `pdfkit`. El checklist de plataformas (`MARKETPLACE_OPTIONS` en `pdfBranding.js`: Mercado Libre, Amazon, Walmart, TikTok Shop, Coppel, Liverpool) es de una sola opción en Cuentas de Plataformas (cada cuenta es de una plataforma), pero de **selección múltiple** en Cuentas Gmail — una misma cuenta Gmail puede dar acceso a varias plataformas a la vez. El teléfono se toma de la línea del celular asignado al empleado (`Asset.specs.lineNumber` vía su `Assignment` activa) — `Employee.phone` casi nunca está capturado — con `Employee.phone` como respaldo. "Tienda/Cuenta/Seller", "Jefe directo", "Rol de acceso", "Vigencia" (y en Gmail, las plataformas) se capturan a mano en un modal antes de generar el PDF, siempre en blanco — no se guardan en la cuenta, viajan como query params solo para esa descarga. La firma de "SISTEMAS" siempre muestra el nombre de quien tenga el correo `gerente.sistemas@selectshop.com.mx` en `Employee.corporateEmails` (nunca el correo).
- `platformAccountsErp.js` (`GET /:id/responsiva`) — **formato distinto**, basado en `Responsiva_Acceso_ERP.docx` (no reutiliza el de marketplaces): título "SOLICITUD Y CARTA RESPONSIVA DE ACCESO AL SISTEMA ERP", sin campo de teléfono, con "Empresa / Razón social" (`employee.businessName`). Sección 2 con **Módulos** del ERP (`MODULE_OPTIONS` — Ventas, Compras, Inventarios/Almacén, Facturación, CxC, CxP, Finanzas/Contabilidad, Bancos/Tesorería, Nómina/RH, Reportes/BI, Otro) de selección múltiple, **Nivel de acceso** (`ACCESS_LEVEL_OPTIONS` — Consulta/Captura-Operación/Autorización-Supervisión/Administrador) de selección única, más Tipo de solicitud (Alta/Modificación/Baja), Empresa(s) del grupo con acceso, Vigencia y Perfil de referencia — todos manuales, nunca se guardan, siempre en blanco. Texto legal (sección 3) y firmas (Usuario responsable / Jefe directo (autoriza) / Sistemas (configura acceso), cada una con su propia línea de fecha en blanco para firmar a mano) tomados verbatim de esa plantilla.
