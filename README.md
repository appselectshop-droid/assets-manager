...
- **PlatformAccountErp** — colección **totalmente separada** de `PlatformAccount`, mismo esquema exacto, para cuentas de plataformas ERP (SAP, NetSuite, Dynamics, etc.). Aislada a propósito: un usuario con solo `canManagePlatformAccountsErp` nunca ve ni toca las cuentas de la colección general, y viceversa.
...
| `platform-accounts-erp` | Mismos endpoints que `platform-accounts` (sin `unregistered-corporate`, no aplica) pero sobre la colección `PlatformAccountErp`, completamente aislada — requiere el permiso `canManagePlatformAccountsErp` |
...
- **PlatformAccountsErp** (`/platform-accounts-erp`) — clon funcional completo de PlatformAccounts (mismas plantillas de UI, filtros, exportación, Responsiva, reciclaje) pero apuntando a la colección aislada `PlatformAccountErp`. Protegida por `PlatformErpManagerRoute` (permiso `canManagePlatformAccountsErp`), pensada para usuarios que **no** deben ver nada más de lo que gestiona Sistemas (Gmail, Cuentas de Plataformas generales, Usuarios, Auditoría).
- Solo `sistemas.2@selectshop.com.mx` (constante `GMAIL_ROOT_EMAIL` en `backend/src/config/permissions.js`) puede otorgar/revocar `canManageGmailAccounts`, `canManagePlatformAccounts` y `canManagePlatformAccountsErp` a otras cuentas, cada uno por separado, desde controles exclusivos en la página de Usuarios.
