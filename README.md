...
- `platformAccounts.js`, `platformAccountsErp.js` y `gmailAccounts.js` (`GET /:id/responsiva`, misma lógica en los tres) — "Solicitud y Carta Responsiva de Cuenta de Acceso a Plataformas Digitales", basada en la plantilla `Responsiva_Cuentas_Plataformas.docx` del usuario. Se llena con los datos del empleado y de la cuenta (nunca la contraseña), y usa `[ ]`/`[X]` en vez de ☐/☒ para los checkbox de plataforma, porque esos caracteres Unicode no se renderizan con la fuente estándar de `pdfkit`. El checklist de plataformas (`MARKETPLACE_OPTIONS` en `pdfBranding.js`: Mercado Libre, Amazon, Walmart, TikTok Shop, Coppel, Liverpool) es de una sola opción en Cuentas de Plataformas/ERP (cada cuenta es de una plataforma), pero de **selección múltiple** en Cuentas Gmail — una misma cuenta Gmail puede dar acceso a varias plataformas a la vez. El teléfono se toma de la línea del celular asignado al empleado (`Asset.specs.lineNumber` vía su `Assignment` activa) — `Employee.phone` casi nunca está capturado — con `Employee.phone` como respaldo. "Tienda/Cuenta/Seller", "Jefe directo", "Rol de acceso", "Vigencia" (y en Gmail, las plataformas) se capturan a mano en un modal antes de generar el PDF, siempre en blanco — no se guardan en la cuenta, viajan como query params solo para esa descarga, porque cada solicitud es para una persona/tienda/combinación distinta. La firma de "SISTEMAS" siempre muestra el nombre de quien tenga el correo `gerente.sistemas@selectshop.com.mx` en `Employee.corporateEmails` (nunca el correo).
