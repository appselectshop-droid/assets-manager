...
- **Qué se agregó en el frontend (`Accessories.jsx`):** al editar un lote existente (`stockTotal != null`), nueva sección "Series registradas en este lote" — mismo input de escaneo + tabla + badge "Listo para escanear" ya usado en el alta nueva, pero aquí para gestionar (agregar/quitar) las series de un lote que ya existe. "Cantidad en stock" se calcula sola de cuántas series hay listadas (se bloquea el campo manual mientras haya al menos una). En la tabla principal, la fila del producto muestra "🔢 N series registradas" cuando aplica.
- **Fix de datos (producción) — piloto, Samsung S24A336NHL:** 2 documentos duplicados (mismo modelo, series `4X4TH4TW300451J` y `4X4TH4TW301683M`) consolidados en 1 solo: `stockTotal: 2`, ambas series en el nuevo campo `serials`, `serialNumber` vacío. Las 2 asignaciones que tenían (1 activa, 1 ya devuelta) se re-apuntaron al documento sobreviviente — ningún empleado perdió su historial. El duplicado se eliminó **como excepción explícita, puntual y autorizada por el usuario** a la regla de "nunca borrar en producción" (ver CLAUDE.md) — el propio usuario lo pidió así para no dejar basura en la auditoría de inventario.
- **Pendiente (mismo proceso, uno por uno):** quedan ~10 grupos más de monitores con duplicados reales, más varios con inconsistencias de captura (typos de marca/modelo, prefijos inconsistentes como "ThinkVision S24i-30" vs "S24i-30") que requieren confirmación puntual antes de fusionar — no se tocaron en esta entrada.
- **Respaldo:** `mongodump` manual antes de cada escritura, subido a S3 (`pre-merge-monitores-2026-09-04_1414.archive.gz`, más los 2 respaldos previos de la sesión).
- **Commit(s):** pendiente (sin commitear aún) para el código; el fix de datos no tiene commit.
...
- **Backend (`backend/src/models/Asset.js`, `backend/src/routes/assets.js`):**
  - `Asset`: nuevos campos `photoData` (Buffer), `photoMimeType`, `photoFileName` — mismo patrón que `Ticket.attachmentData` (binario en Mongo, no en disco).
  - `POST /assets/batch`: alta de N activos idénticos (mismos datos comunes/specs), uno por cada serie en `serialNumbers[]`; valida duplicados igual que `POST /assets`.
  - `PUT /assets/:id/transfer`: activo único → solo mueve `location`; lote con transferencia parcial → resta del origen y crea/incrementa un gemelo en la sucursal destino (valida contra lo disponible, igual que `POST /assignments`).
  - `POST /assets/:id/photo` (multer memoryStorage, mismo `ALLOWED_..._MIME` que Tickets) y `GET /assets/:id/photo` para subir/servir la foto aparte del alta/edición.
  - Nuevo `LIST_EXCLUDE_FIELDS = '-photoData'` aplicado en `GET /`, `GET /:id`, `PUT /:id` y `split-line` para no pesar esas respuestas con el binario.
- **Frontend (`frontend/src/pages/Assets.jsx`, `Assets.module.css`):**
  - `AssetModal`: toggle "Único por número de serie" / "Por cantidad/lote" (solo al registrar, no al editar — el modo queda fijo una vez creado). Modo serie: campo de cantidad de referencia + tabla de series alimentada por un input que escucha `Enter` (compatible con lector de código de barras) o el botón "+ Agregar"; alerta si alguna serie ya existe. Modo lote: campo numérico "Cantidad" (mapea a `stockTotal`), oculta No. de serie.
  - Sección de foto (input `capture="environment"` + preview) en el mismo modal, para ambos modos y también al editar.
  - Nuevo `TransferModal`: botón "🚚 Transferir" junto a "Editar" en cada fila; selecciona sucursal destino (catálogo `oficina` existente) y, si es lote, cantidad a mover (tope = disponible).
- **Fuera de alcance de esta entrega (decisión consciente, no pendiente por olvido):** no se tocó `Accessories.jsx` (modal casi idéntico, pero el pedido hablaba de "activos"); no se agregó envío de la misma acción a selección múltiple de la tabla (el transfer es por fila); no se permite asignar a un empleado en el mismo alta cuando el modo serie registra más de 1 unidad (asignación individual sigue funcionando igual que siempre al editar cada una después).
- **Verificación:** `node --check` en `Asset.js`/`assets.js` sin errores; `npm run build` del frontend sin errores; arranque del backend en local confirma que Express registra todas las rutas y el modelo sin crashear (el único error fue `ECONNREFUSED` al Mongo por no tener el túnel SSH activo — infraestructura, no código). No se creó ningún dato de prueba en la base de producción.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-26 — Regla nueva en CLAUDE.md: avisar ANTES de crear algo que después haya que borrar
- **Qué pasó:** el usuario preguntó por un incidente viejo del CHANGELOG (2026-07-10: un `AuditLog.deleteMany({entity:'ticket'})` sin acotar por folio, al limpiar tickets de prueba propios, se llevó también 2 entradas reales de auditoría de otra persona). Pidió primero que Claude dejara de borrar cualquier cosa; se refinó en la misma conversación a una regla de aviso previo en vez de una prohibición total.
- **Qué se agregó:** nueva sección "⚠️ REGLA FIJA — Avisar ANTES de crear algo que después haya que borrar" en `CLAUDE.md`, mismo formato que la regla ya existente de no escribir en producción sin avisar: antes de crear cualquier dato/registro/archivo que después haya que borrar o deshacer, avisar qué exactamente, dónde (colección/entorno — recordar que no hay staging), y cómo se va a limpiar después, y esperar confirmación explícita. Si algo queda sin avisar antes, no borrarlo por cuenta propia — reportarlo y que decida el usuario.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-26 — CLAUDE.md: explicitar que "solo en local" también requiere avisar antes de mover la BD
- **Qué pasó:** el usuario preguntó si la regla de avisar antes de mover la BD ya cubría el caso de hacerlo "en local" — la cobertura existía por implicación (sección "Contexto por qué esto importa", que explica que no hay staging y `MONGO_URI` local apunta a la BD real) pero no como punto explícito en la lista de "sin excepción, incluso si...".
- **Qué se agregó:** nuevo punto textual en esa lista: "El movimiento se vaya a hacer 'solo en local' — no hay ambiente de staging, `MONGO_URI` en local (incluso por el túnel SSH de desarrollo) apunta a la misma base de datos real de producción, sin excepción."
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-26 — CLAUDE.md: link de la matriz de pruebas de Felipe (SharePoint) + comando "corrige lo de la matriz"
- **Qué pasó:** el usuario dio el link real del Excel de SharePoint donde Felipe (`sistemas.4@selectshop.com.mx`) lleva su matriz de pruebas, y pidió que cada vez que diga "corrige lo de la matriz" se revise ese archivo y se corrija todo lo que Felipe haya reportado — mismo patrón ya usado antes (ver "FIX: 4 bugs del Calendario (matriz de pruebas de Felipe)", 2026-08-19), ahora formalizado como comando repetible.
- **Qué se agregó:** nueva sección "Matriz de pruebas de Felipe (SharePoint)" en `CLAUDE.md` con el link y la instrucción de qué hacer al recibir ese comando.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-26 — FIX (matriz de Felipe, BUG-07): Tiempos Comprometidos de ERP con fecha pasada y desfase de -6h
- **Qué pasó:** Felipe reportó en su matriz de pruebas que "Tiempos Comprometidos" (Respuesta/Resolución comprometida de tickets ERP) permitía guardar una fecha/hora ya pasada, y que el valor guardado salía con un desfase de -6 horas respecto a lo capturado.
- **Causa raíz del desfase:** el `<input type="datetime-local">` manda un string sin zona horaria (ej. `"2026-08-25T14:30"`); `PUT /:id/erp-sla-custom` lo parseaba con `new Date(...)` directo — como el EC2 corre en UTC, Node interpretaba esos dígitos como hora UTC en vez de hora de México, guardando siempre 6 horas antes de lo capturado. El mismo bug existía en `PUT /:id/extend-sla` y `PUT /:id/extend-provider-sla` (mismo tipo de input), aunque Felipe solo probó Tiempos Comprometidos.
- **Qué se corrigió:** nuevo helper `parseMx()` en `backend/src/utils/dateFormat.js` (inverso de `formatMx`, asume que el string naive representa hora de México UTC-6 fijo — mismo criterio ya usado en `calendarActivities.js` para BUG-01/02) — usado ahora en las 3 rutas (`erp-sla-custom`, `extend-sla`, `extend-provider-sla`) en vez de `new Date()` directo. Se agregó también la validación de "debe ser fecha futura" en `erp-sla-custom` (ya existía en las otras dos). En el frontend (`TicketDetailModal.jsx`): nuevo helper `toMxDatetimeLocalInput()` en `ticketShared.js` para precargar el formulario de "Cambiar tiempos" en hora de México (antes usaba `toISOString()`, UTC) — y se agregó `timeZone: 'America/Mexico_City'` explícito a las 2 líneas de solo-lectura que mostraban las fechas ya guardadas.
- **Verificación:** `node -c` en backend sin errores; `npm run build` de frontend sin errores; prueba manual de round-trip de `parseMx`/`toMxDatetimeLocalInput` con Node (`TZ=UTC`, igual que el EC2): captura de "14:30" → se guarda como el instante UTC correcto (20:30Z) → `formatMx` y `toMxDatetimeLocalInput` lo vuelven a mostrar como "14:30", ya no "08:30".
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-27 — FIX (matriz de Felipe, BUG-08): link de Telegram/correo a un ticket de Solicitud de Pagos no abría nada
- **Qué pasó:** Felipe reportó que al generar un ticket de Solicitud de Pagos, el folio se creaba pero "no aparece en la app la solicitud".
- **Investigación:** que Solicitud de Pagos (apartados Centro de Costos/Motivo de Pago/Alta de Proveedores, `requestAudience:'externo'`) NO aparezca en el Tablero de Sistemas ni en "Mis Tickets" **es diseño intencional** (ver entrada 2026-07-28) — eso no es el bug. El bug real: ese mismo diseño prometía que el ticket "sigue abierto por su `_id` directo (ej. desde el link del correo/Telegram)" — pero `frontend/src/pages/TicketsLayout.jsx` resolvía el `?ticket=<id>` del link buscándolo dentro de `GET /tickets` (la lista que el backend ya filtra excluyendo `requestAudience:'externo'`), así que `found` siempre era `undefined` y la URL se limpiaba en silencio sin abrir nada ni avisar del error — ni Felipe ni ningún admin, ni siquiera el Gerente de Sistemas, podían abrirlo desde ahí. El backend (`GET /tickets/:id`, `canViewTicket()`) sí lo serviría sin problema — nunca se le pedía.
- **Qué se corrigió:** `TicketsLayout.jsx` ahora, si el ticket del `?ticket=<id>` no está en la lista ya cargada, hace `GET /tickets/:id` directo antes de rendirse — restaura la vía de rescate ya documentada, sin cambiar el filtro del Tablero (Solicitud de Pagos sigue sin listarse ahí, solo ahora sí se puede abrir por su link).
- **Fuera de alcance de este fix (documentado, no corregido):** en "Mis Solicitudes" del portal del empleado, la fila de una Solicitud de Pagos se ve pero no es clickable (sin chat/detalle) — no se tocó, porque Contabilidad/Pagos no tienen cuenta en el sistema y el diseño actual es que se resuelva por correo, no por chat en la app.
- **Verificación:** `npm run build` sin errores. No requirió cambios de backend (la ruta `GET /tickets/:id` ya soportaba esto).
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-27 — FIX (matriz de Felipe, BUG-09): "Tiempo de resolución" ya no se asigna hasta que se toma el ticket
- **Qué pasó:** Felipe reportó que el "Tiempo de resolución" se calculaba automáticamente desde que el ticket llegaba, sin importar que nadie lo hubiera tomado todavía — debía calcularse solo hasta que el ticket pasa a "En proceso/Atención".
- **Causa raíz:** `applySlaCategory()` (`backend/src/utils/slaClassifier.js`) calculaba `responseDueAt` y `resolutionDueAt` siempre juntos, en el mismo instante (al crear el ticket o al clasificarlo), sin importar el `status`.
- **Qué se corrigió:** `applySlaCategory()` ahora solo calcula `resolutionDueAt` si el ticket ya está en `en_proceso` (o más adelante) — si sigue `abierto`, lo deja en `null`, igual que ya pasaba con un ticket sin clasificar (todos los consumidores del campo ya toleraban `null`, no hizo falta tocar nada más). Nueva función `assignResolutionDueAt(ticket)` en el mismo archivo, llamada desde los 4 puntos donde un ticket pasa de `abierto` a `en_proceso` en `tickets.js`: `PUT /:id/assign`, `PUT /:id/escalate` (persona y proveedor) y el auto-assign de `POST /:id/reply` — mismo criterio que ya existía para `providerSlaDueAt` (se calcula al escalar, no al crear). En `TicketDetailModal.jsx` se agregó un aviso ("Todavía sin tiempo de resolución — se calcula en cuanto alguien tome el ticket") para que se vea intencional, mismo copy que ya usaba el bloque de ERP para su propio caso de "sin tiempos".
- **Fuera de alcance:** tickets ERP/`reporte_erp` no se tocaron — ya funcionaban distinto (ambas fechas se capturan a mano en `PUT /:id/erp-sla-custom` cuando el proveedor da fecha real, tiene sentido que nazcan juntas ahí).
- **Verificación:** `node -c` en backend sin errores; `npm run build` de frontend sin errores; prueba con Node de la función pura (`applySlaCategory`/`assignResolutionDueAt`) simulando los 4 casos: creación en abierto (solo responseDueAt), toma → se calcula resolutionDueAt, segunda toma no lo pisa, clasificación directa ya en_proceso calcula ambos juntos — los 4 se comportaron como se esperaba.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-31 — FIX urgente: BI (y cualquier área) puede reenviar un ticket que ya recibió a otra área/persona
- **Qué pasó:** caso real reportado por el usuario: un ticket de Isaac Chávez se escaló a BI y, tras hablar con el empleado, resultó ser de Sistemas — BI no tenía forma de mandarlo de vuelta, aunque "Sistemas" siempre estuvo en su propia lista de destinos válidos de escalamiento.
- **Causa raíz:** `PUT /:id/escalate` (`backend/src/routes/tickets.js`) rechazaba CUALQUIER escalamiento nuevo una vez que `ticket.escalated` era `true`, sin importar el destino — la única excepción era "último recurso a Proveedor externo" (regla del 2026-08-06, pensada para evitar *desescalar* un ticket ya resuelto, no para bloquear reenrutar entre áreas). Como escalar A un área (`kind:'area'`) también pone `escalated:true`, cualquier ticket que llegara a la cola de un área quedaba congelado ahí para siempre — sin distinguir "nadie lo ha tomado todavía" (se puede reenrutar sin problema) de "ya se hizo cargo una persona o un proveedor" (ahí sí no debe deshacerse).
- **Qué se corrigió:** nueva excepción `isAreaFloating` en el guard — mientras el ticket sigue en `escalationType === 'area'` (flotando en la cola de un área, sin persona asignada todavía), se permite volver a escalar a otra área o directo a una persona, con el mismo menú de siempre. Una vez escalado a PERSONA o a PROVEEDOR, la restricción original sigue aplicando sin cambios. En el frontend (`TicketDetailModal.jsx`), nuevo botón "🔀 Reenviar a otra área / persona" visible cuando el ticket está en esa situación, reusando el mismo `<select>`/lógica que el escalamiento inicial (menos la opción de Proveedor, que ya tiene su propio botón dedicado).
- **Verificación:** `node -c` en backend sin errores; `npm run build` de frontend sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-31 — FIX: gate "¿cuenta nueva o ya existe?" en Solicitar Cuenta (Mesa de Ayuda)
- **Qué pasó:** el usuario reportó ~7 solicitudes reales de gente que en realidad tenía un problema con una cuenta YA EXISTENTE (contraseña, bloqueo, permisos) pero usó por error el formulario de "Solicitar Cuenta" (que es solo para pedir una cuenta que todavía no existe) en vez de "Tengo un problema" → Cuenta/Acceso.
- **Qué se corrigió:** `SolicitarCuenta.jsx` ahora muestra primero una pantalla obligatoria "¿Es una cuenta nueva o ya existe?" antes de dejar ver el formulario — si eligen "Ya tengo la cuenta, pero no puedo entrar / algo no funciona", ni siquiera se les deja continuar: se les manda directo a `/mesa-de-ayuda/reportar-ticket?tipo=cuenta_acceso` (deep-link ya soportado nativamente por `ReportarTicket.jsx`, salta directo al paso de "Cuenta / Acceso — ¿cuál es el problema?", sin pasar por elegir categoría). Solo con "Es una cuenta nueva" se avanza al formulario de siempre.
- **Verificación:** `npm run build` sin errores.
- **De paso, mismo pedido:** en `MesaDeAyuda.jsx`, la tarjeta que lleva a este formulario ("Acceso a un sistema o correo" / "Gmail, una plataforma de venta o el ERP.") se renombró a **"Solicitar una cuenta nueva"** / "Gmail, una plataforma de venta o el ERP — solo para dar de alta un acceso que todavía no existe." — para que quede claro desde antes de entrar, no solo dentro del formulario.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-31 — FIX: tarjeta "Equipo, accesorio o servicio" de Mesa de Ayuda aclarada a "Solicitar un Recurso"
- **Qué pasó:** mismo pedido/criterio que la tarjeta de cuentas de la entrada anterior — el usuario pidió que la tarjeta que lleva a `SolicitarRecurso.jsx` dijera explícitamente que se está pidiendo un **recurso**, no solo la lista de ejemplos (monitor, mouse, etc.), para que no se confunda con reportar una falla de un equipo que ya se tiene.
- **Qué se corrigió:** `MesaDeAyuda.jsx` — título de la tarjeta cambiado de "Equipo, accesorio o servicio" a **"Solicitar un Recurso"** (mismo nombre que ya usa la página destino, "Solicitud de Recursos"), descripción aclarada a "Equipo, accesorio o servicio nuevo — monitor, mouse, línea telefónica, software o licencia...".
- **Verificación:** `npm run build` sin errores.
- **Commit(s):** pendiente (sin commitear aún).
...
### 2026-08-31 — FIX: aplicaciones externas (no le competen a Sistemas/ERP/BI/Infra) muestran "Sin status" en Mis Solicitudes
- **Qué pasó:** el usuario pidió que las aplicaciones externas (las que no le competen a Sistemas — ERP, BI e Infra) no muestren un status calculado.
- **Investigación:** en `MisSolicitudes.jsx`, `normalizeExternalRequest()` alimenta la lista de solicitudes con `requestAudience:'externo'` (`GET /tickets/mine/external-requests`) — por construcción, TODO lo que llega ahí ya es una aplicación externa (Worky, apartados de Solicitud de Pagos a Contabilidad/Pagos). Pero se le calculaba un status con `BI_STATUS_CONFIG[t.status]` (pendiente/en proceso/resuelto/cerrado) igual que a Soporte BI, aunque nadie del sistema esté actualizando ese estatus de verdad — Contabilidad/RH ni siquiera tienen cuenta en Assets Manager (ver entradas 2026-07-28/2026-08-27), así que mostrarlo era engañoso.
- **Qué se corrigió:** nuevo `NO_STATUS_CONFIG = { label: 'sin status', pillClass: 'pillGray' }`, usado siempre en `normalizeExternalRequest()` en vez de `BI_STATUS_CONFIG[t.status]`.
- **Verificación:** `npm run build` sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-31 — FIX: al iniciar sesión en Mesa de Ayuda, siempre cae en "Mis Solicitudes"
- **Qué pasó:** el usuario pidió que al entrar/iniciar sesión en Mesa de Ayuda, el empleado siempre caiga en la página de Solicitudes, no en la principal (las tarjetas de Solicitar Cuenta/Recurso/Ingreso/Tengo un problema/etc.).
- **Investigación:** el flujo normal de entrada es el login inline dentro de `MesaDeAyuda.jsx` (`WelcomeScreen`) — antes su `onSuccess` solo hacía `setEmployeeUser`, así que la misma página se volvía a renderizar ya con sesión, mostrando las tarjetas. Un segundo punto (`EmployeeLogin.jsx`, la página completa de login usada cuando `EmployeeRoute` redirige por falta de sesión desde una ruta protegida, o por link directo) ya no apuntaba a las tarjetas — su default sin `?next=` era Mis Tickets, no Solicitudes.
- **Qué se corrigió:** `MesaDeAyuda.jsx` — el `onSuccess` del login inline ahora, además de `setEmployeeUser`, navega a `/mesa-de-ayuda/mis-solicitudes` (`replace`). `EmployeeLogin.jsx` — el default cuando no hay `?next=` cambió de `/mesa-de-ayuda/mis-tickets` a `/mesa-de-ayuda/mis-solicitudes`, para que el destino sea consistente sin importar por cuál de los 2 logins entre. Cuando `EmployeeRoute` ya manda un `?next=` explícito (caso normal de ruta protegida), ese destino se sigue respetando igual — no se tocó.
- **Verificación:** `npm run build` sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-31 — FIX: evidencia/adjuntos de Tickets se abren en visor embebido, no en pestaña externa
- **Qué pasó:** el usuario pidió que abrir la evidencia de un ticket (o el comprobante bancario, o un PDF adjunto en el chat) abriera en una ventana emergente DENTRO de la app, mismo criterio que ya usan las Responsivas — no en una pestaña nueva del navegador.
- **Investigación:** 3 lugares usaban `window.open`/`<a target="_blank">`: `openAttachment()` y `openBankProof()` en `TicketDetailModal.jsx`, y la rama "no es imagen ni video" (típicamente un PDF) de `MessageAttachmentImage.jsx` (usada en el chat de tickets). Quedaron fuera a propósito cuando se hizo el visor de PDF de Responsivas (2026-08-17) — ahora sí se pidieron. La evidencia/adjuntos de tickets pueden ser imagen O PDF según lo que suba quien reporta (`ALLOWED_ATTACHMENT_MIME` en el backend acepta ambos).
- **Qué se corrigió:** `PdfViewerModal.jsx`/`usePdfViewer.js` ahora aceptan un `mimeType` opcional — con `image/*` muestran la imagen centrada en vez del `<iframe>` de PDF (mismo modal, sin construir un visor nuevo desde cero). Sin `mimeType` (todos los usos previos: Responsivas, Solicitudes de Cuenta, etc.) el comportamiento queda idéntico a antes. `openAttachment`/`openBankProof` en `TicketDetailModal.jsx` ahora usan `usePdfViewer`/`PdfViewerModal` en vez de `window.open`. `MessageAttachmentImage.jsx` — la rama de PDF ahora abre `PdfViewerModal` en vez de `<a target="_blank">` (la imagen ya tenía su propio lightbox desde 2026-08-05, no se tocó).
- **Verificación:** `npm run build` sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-31 — FEATURE (con expectativas honestas): `capture_links` en ambos manifests, para que el link "Ver ticket" del correo abra la PWA instalada en vez del navegador
- **Qué pasó:** el usuario reportó que en Windows, el botón "Ver ticket en el panel" del correo de notificación abre el navegador en vez de la PWA (Sistema de Tickets) ya instalada.
- **Investigación:** esto ya se había intervenido una vez (2026-08-06, se agregó `launch_handler`) — pero `launch_handler` solo decide qué pasa DESPUÉS de que el sistema operativo ya eligió abrir la app, no si la abre en primer lugar. Faltaba `capture_links`, que sí es la señal real para que Chrome/Edge capturen un link dentro del scope de una PWA instalada. El link en sí (`https://activos.eup.com.mx/login?next=%2Ftickets`, armado en `tickets.js`) solo lo reciben cuentas de Sistemas, nunca empleados — cae dentro del scope `"/"` de la PWA de admin.
- **Qué se agregó:** `capture_links: 'existing-client-navigate'` en el manifest del panel (`vite.config.js`) y en el de Mesa de Ayuda (`public/manifest-mesa-de-ayuda.webmanifest`), junto al `launch_handler` que ya existía.
- **Expectativas honestas, sin prometer de más:** funciona mejor en Windows/desktop con Chrome o Edge como navegador default (el caso reportado). En Android/Chrome puede mostrar un selector "¿Abrir en la app o en Chrome?" en vez de abrir directo, y depende de qué app renderice el correo (si Gmail/Outlook para Android usan su propio navegador interno, ni siquiera llega a aplicar). **En iOS no tiene ningún efecto** — es una limitación de Apple, no de este código: iOS nunca deja que un link externo abra una PWA instalada.
- **Importante para probarlo:** el manifest de una PWA ya instalada no se re-evalúa solo — hay que desinstalar y volver a instalar la app (o esperar a que el navegador la re-sincronice sola, que no tiene un tiempo fijo) para que `capture_links` tome efecto.
- **Verificación:** `npm run build` sin errores; confirmado con el manifest generado (`dist/manifest.webmanifest`) que el campo `capture_links` sí queda incluido.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-09-01 — FEATURE (matriz de Felipe, sugerencia #26): selector "ERP afectado" al reportar ticket
- **Qué pasó:** el formulario de "Tengo un problema" solo tenía la categoría genérica "ERP" (fusionada dentro de Aplicaciones desde 2026-07-22), sin forma de decir cuál sistema — pendiente de decisión de producto documentado desde entonces.
- **Decisión tomada con el usuario:** el catálogo del desplegable es "por empresa del grupo" (ERP SelectShop/Nexustore/Medicalstore/Tlab — mismo catálogo que ya usa Solicitud de Cuenta ERP), no por módulo Aspel (SAE/COI/NOI, que no existía en el código todavía).
- **Qué se corrigió:** el catálogo `ERP_SYSTEM_CATALOG` vivía duplicado (frontend `SolicitarCuenta.jsx` + backend `accountRequests.js`) — se extrajo a un solo lugar compartido (`frontend/src/config/erpSystems.js` y `backend/src/config/erpSystems.js`), y ambos archivos viejos ahora importan de ahí. Nuevo campo `Ticket.erpSystem` (obligatorio si `ticketType==='erp'`, validado contra el catálogo cerrado en `POST /tickets/mine`). `ReportarTicket.jsx` — nuevo `<select>` "ERP afectado" cuando `forcedTicketType==='erp'`. `TicketDetailModal.jsx` — se muestra el dato en el detalle del ticket.
- **Verificación:** `node -c` en backend sin errores; `npm run build` de frontend sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-09-01 — FIX (matriz de Felipe, BUG-11): Solicitud de Cuenta ERP redirigida a ticket ahora enruta a ERP, no a Sistemas
- **Qué pasó:** Felipe reportó que al redirigir una Solicitud de Cuenta ERP (`requestType:'platform_erp'`) a un ticket, el ticket se enrutaba a Sistemas/IT en vez de al área de ERP.
- **Causa raíz:** `PUT /account-requests/:id/redirect-to-ticket` (`accountRequests.js`) siempre creaba el ticket con `ticketType:'cuenta_acceso'` sin importar el `requestType` original de la solicitud — `canViewTicket()`/`isErpOnlyUser()` en `tickets.js` solo reconocen `ticketType` `'erp'`/`'reporte_erp'` como del área de ERP, así que el ticket caía en la cola general de Sistemas.
- **Qué se corrigió:** nueva variable `isErpRequest = request.requestType === 'platform_erp'` — si es cierto, el ticket nace con `ticketType:'erp'` (en vez de `'cuenta_acceso'`) y se clasifica con SLA "Cuentas Críticas / ERP-SAE" (en vez de "Cuentas y Accesos").
- **Verificación:** `node -c` sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-09-01 — FEATURE (matriz de Felipe, BUG-10): SLA de tickets respeta el horario laboral de Sistemas
- **Qué pasó:** Felipe reportó que los tiempos SLA se calculaban en tiempo corrido (24/7), sin pausar fuera de horario laboral ni trasladar el remanente al siguiente día hábil.
- **Reglas reales confirmadas con el usuario** (el caso de prueba de Felipe solo era ilustrativo, no las reglas reales): horario de Sistemas **8:00–19:00**, hábil **lunes a viernes** (sábado/domingo no cuentan), **sin excluir festivos por ahora**, aplica **por igual** a `responseDueAt`, `resolutionDueAt` y `providerSlaDueAt` (no solo a tickets normales de Sistemas).
- **Qué se agregó:** `backend/src/utils/businessHours.js` — `addBusinessMinutes(startDate, minutes)`, mismo truco de zona horaria ya usado en `calendarActivities.js` (México UTC-6 fijo, sin DST, usando métodos `getUTC*`/`setUTC*` para no depender del huso del proceso). Si el punto de partida cae fuera de jornada (noche, fin de semana), el conteo arranca hasta el siguiente inicio hábil. Usado en `applySlaCategory()`/`assignResolutionDueAt()` (`slaClassifier.js`, ya no `base + minutos*60000` en tiempo corrido) y en el cálculo de `providerSlaDueAt` al escalar a proveedor (`tickets.js`). No hizo falta tocar `isOverdue()`/`autoCloseAbandonedOverdue()` — siguen comparando `now > dueAt`, y `dueAt` ya llega correctamente calculado.
- **Verificación:** `node -c` sin errores; prueba con Node de `addBusinessMinutes` con 4 casos (dentro de jornada, cruza a las 19:00 → salta a las 8:00 del día hábil siguiente, arranca en fin de semana → salta a lunes 8:00, y un total que cruza 2+ días hábiles completos) — los 4 dieron el resultado exacto esperado.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-09-03 — FEATURE: más permisos para el líder de ERP, acotados solo a ERP
- **Qué pasó:** pedido explícito del usuario: "dale permisos como al líder de infraestructura pero solo con respecto al ERP" — Miguel García (lider.infra.soporte) es `role:'admin'` completo (poder sobre TODO el sistema); darle lo mismo a Leonardo Villareal (lider.erp) habría sido excesivo, así que se replicó el patrón ya usado para que él elimine tickets de su área (`isErpLeader`, por correo real) y se extendió a 3 acciones más, todas acotadas a ERP.
- **Qué se agregó:**
  - `isErpLeader`/`LIDER_ERP_EMAIL` se movieron de `tickets.js` a `backend/src/config/permissions.js` (compartido) para reusarse en los archivos nuevos, sin tocar `tickets.js`.
  - **Eliminar cuentas ERP** (`platformAccountsErp.js`, antes `adminOnly`) — ahora admin o `isErpLeader`; toda esa colección ya es 100% ERP, no hizo falta acotar más.
  - **Eliminar responsivas de cuenta ERP** (`responsivaArchive.js`, antes `adminOnly` en ambas rutas de borrado) — ahora admin, o `isErpLeader` **solo si** `doc.type === 'cuenta_plataforma_erp'` (esta colección mezcla activos/Gmail/plataformas/ERP, así que sí hacía falta acotar por tipo). De paso: la lista (`GET /`) y la descarga/gestión de la firmada ahora también le muestran a Leonardo TODAS las responsivas de cuenta ERP (no solo las que él mismo generó — antes solo veía las suyas, y no podría haber administrado el duplicado de Yocelin Contla de la Parte 1 de abajo).
  - **Ver Auditoría, acotada a ERP** — nueva ruta `AuditRoute` en `App.jsx` (antes solo `AdminRoute`) + ítem en el sidebar (`Layout.jsx`, solo visible para `lider.erp@selectshop.com.mx` exacto, no cualquier analista). El backend (`audit.js`) fuerza siempre `entity:'cuenta_plataforma_erp'` para él, sin importar qué mande de query param — no ve auditoría de nada más. **Hallazgo de paso:** `GET /api/audit` no tenía NINGÚN control de acceso en el backend (solo `auth`) — cualquier usuario autenticado podía pedir la auditoría completa del sistema llamando la API directo, el único freno real era el frontend. Se cerró ahora de una vez.
- **Verificación:** `node -c` en los 4 archivos de backend tocados sin errores; `npm run build` de frontend sin errores.
- **Commit(s):** pendiente (sin commitear aún).

### 2026-09-03 — Hallazgo (sin corregir aún): GET con efecto secundario + retry automático duplica responsivas
- **Qué pasó:** el usuario reportó (captura de pantalla) 2 responsivas idénticas de cuenta ERP para Manuel Alejandro Montes Suárez (#71685), generadas con 14 segundos de diferencia por Yocelin Contla.
- **Causa raíz encontrada:** `GET /:id/responsiva` (`platformAccountsErp.js`) archiva un `ResponsivaArchive` nuevo en CADA llamada, sin checar si ya existe uno reciente — y el interceptor de axios (`frontend/src/services/api.js`) reintenta automáticamente cualquier GET que truene por wifi inestable, asumiendo que GET siempre es idempotente. Para esta ruta esa asunción es falsa: si el servidor ya archivó pero la respuesta se perdió en el camino (típico en wifi de sucursal), el reintento crea un segundo documento idéntico. Puede repetirse en cualquier responsiva del sistema, no solo esta.
- **Pendiente, no corregido en esta sesión:** hacer la ruta idempotente (ej. deduplicar por empleado+tipo+ventana de tiempo antes de archivar) o excluirla del retry automático del interceptor — decidir con el usuario el criterio antes de tocarlo.
- **El duplicado real de Manuel Montes** (`_id: 6a63b21f...`, el que nunca recibió firma) — pendiente de confirmación explícita del usuario para borrarlo (regla fija de `CLAUDE.md`); no se tocó nada en Mongo.

### 2026-09-03 — ⚠️ Corrección en producción: desfase de -6h histórico en 18 tickets ERP (Tiempos Comprometidos/extensiones de SLA)
- **Qué pasó:** al confirmar BUG-07 (ver entrada del 2026-08-27), se detectó que el bug de desfase estuvo activo desde antes del fix (2026-08-31) — cualquier fecha capturada a mano en "Tiempos Comprometidos" (`PUT /:id/erp-sla-custom`), "Extender SLA" (`PUT /:id/extend-sla`) o "Extender SLA con Proveedor" (`PUT /:id/extend-provider-sla`) antes de esa fecha quedó guardada 6 horas antes de lo que la persona realmente tecleó.
- **Investigación (solo lectura primero):** se encontraron 18 tickets afectados — los 18 ya estaban `cerrado`/`resuelto` (ninguno abierto/en proceso), así que la corrección no cambió nada visible en el día a día, solo la exactitud del historial para reportes de cumplimiento SLA (`TicketsSLA.jsx`).
- **⚠️ Escritura en producción, con confirmación explícita del usuario:** backup fresco (`mongodump` completo, subido a `s3://eup-assets-manager-backups/mongo/manual/pre-sla-fix-20260903-194216.archive.gz`) antes de escribir. 34 operaciones vía script (no valores hardcodeados a ciegas: cada operación leyó el valor actual, lo validó contra lo ya investigado, y solo entonces escribió +6h):
  - 11 tickets: `responseDueAt`/`resolutionDueAt` (TICK-CBE68D, A54793, 4EB933, BEC48D, 42730B, F08D99, E4CCBA, FAFB90, D0566E, C5B8C6, 4A8AB4).
  - 6 tickets: `slaExtensions[].newResolutionDueAt` + `resolutionDueAt` actual cuando coincidía (TICK-0588CD, 79DF8B ×2 extensiones, E8885B, 456398, 2F7C4E, C620D0).
  - 4 tickets: `providerSlaExtensions[].newProviderSlaDueAt` + `providerSlaDueAt` actual cuando coincidía (TICK-0588CD, D0566E ×2, B01D01 ×2, C5B8C6).
  - En 2 casos con extensiones encadenadas (TICK-79DF8B, TICK-D0566E/B01D01) también se corrigió el `previousXxxDueAt` de la extensión siguiente, para que el historial quedara internamente consistente. **No se tocaron** los `previousXxxDueAt` que venían de un cálculo automático del sistema (esos nunca tuvieron el bug — solo lo tienen los valores tecleados a mano).
- **Verificación:** releído `TICK-F08D99` y `TICK-79DF8B` después de escribir — coinciden exacto con lo planeado, y el encadenado quedó consistente.

### 2026-09-03 — Eliminar responsivas de cuenta ERP, ampliado también a la analista (Yocelin Contla)
- **Qué pasó:** mismo día, el usuario pidió que además del líder, la analista de ERP también pudiera eliminar responsivas — "es quien más las genera, y por lo tanto quien más se topa con duplicados reales que hay que limpiar" (el propio caso real de Manuel Montes de hoy, generado por ella).
- **Qué se corrigió:** `responsivaArchive.js` — nueva `canDeleteErpResponsiva(user)` (`!!user.canManagePlatformAccountsErp`, el mismo permiso compartido que ya tiene cualquiera de ERP, no exclusivo del líder) usada en `canDelete()`, en vez de `isErpLeader`. Sigue acotado a `doc.type === 'cuenta_plataforma_erp'` — no puede borrar responsivas de otro tipo. `ResponsivasArchive.jsx` — el botón "Eliminar" ahora se muestra con el mismo criterio (`currentUser.canManagePlatformAccountsErp`), no solo para el correo del líder. La lista (`GET /`) sigue igual: el líder ve TODAS las de cuenta ERP, cualquier otro usuario (incluida la analista) solo ve las que él mismo generó — para el caso reportado eso es suficiente, porque Yocelin fue quien generó el duplicado.
- **Verificación:** `node -c` sin errores; `npm run build` sin errores.

### 2026-09-03 — Auditoría de Leonardo re-acotada: por EQUIPO (todo lo que hace Yocelin), no por tipo de entidad
- **Qué pasó:** mismo día, el usuario pidió explícitamente "que Leonardo vea todo lo que hace Yocelin, como el de auditoría que tenemos nosotros" — la implementación de la mañana (acotada a `entity:'cuenta_plataforma_erp'`) se quedaba corta: no mostraba nada de lo que Yocelin hace en Tickets ERP ni en Responsivas (y de paso se descubrió que **eliminar/subir la firmada de una responsiva nunca se registró en Auditoría**, para nadie, desde que existe ese módulo).
- **Qué se corrigió:**
  - `responsivaArchive.js` — se agregó `logAction()` a las 3 acciones que no lo tenían: subir firmada, quitar firmada, eliminar del archivo. Usa `doc.type` directo como `entity` (coincide 1 a 1 con el enum de `AuditLog`: activo/cuenta_plataforma/cuenta_plataforma_erp/cuenta_gmail).
  - `audit.js` — reescrito el criterio de acceso para no-admin: en vez de forzar `entity`, ahora fuerza `userId: {$in: <ids de todo el equipo de ERP>}` (`User.find({canManagePlatformAccountsErp: true})`, resuelto en cada request, sin caché — el equipo es chico). El filtro de `entity` sigue disponible como narrowing opcional (ej. Leonardo puede filtrar "solo cuentas ERP" dentro de su equipo si quiere), pero ya no es la única dimensión de acceso. Aplica a las 3 rutas (`GET /`, `/counts-by-action`, `/users`).
  - `Layout.jsx` — descripción del ítem del sidebar actualizada a "Bitácora del equipo de ERP".
  - `canDeleteErpResponsiva`/permiso de eliminar responsivas del turno anterior **no cambia** — sigue siendo cualquiera con `canManagePlatformAccountsErp` (Leonardo y Yocelin).
- **Verificación:** `node -c` sin errores; `npm run build` sin errores.
- **Commit(s):** pendiente (sin commitear aún).
...
- **Qué se creó (frontend):** `pages/AssetBajas.jsx` (listado con tabs por motivo, total vendido del filtro actual, modal de detalle, descarga de PDF vía `usePdfViewer`/`PdfViewerModal` como Envíos) y `components/CreateAssetBajaModal.jsx` (buscador de activo por marca/modelo/serie/etiqueta — incluye activos asignados, con aviso de que se devuelven automáticamente —, condición, checkbox de datos borrados, motivo, y si es Venta: tipo de comprador empleado/externo con buscador de empleado o campos de identificación si es externo, monto, forma de pago, fecha, referencia). Ruta `/asset-bajas` en `App.jsx` (`AdminRoute`) e ítem "Bajas de Activos" en el sidebar (`Layout.jsx`, sección Catálogos y Activos, junto a Asignaciones — solo admin).
- **Decisiones tomadas con el usuario (mismas ya validadas en la entrada de la plantilla Word, reusadas aquí):** autoriza el Gerente de Sistemas; comprador puede ser empleado interno o externo; se captura monto+forma de pago, condición del equipo, y la cláusula de "tal cual sin garantía" va en el PDF solo si el motivo es Venta.
- **Verificación:** `node -c` sin errores en los 5 archivos nuevos/editados del backend; `npm run build` del frontend sin errores. **Pendiente de probar en el navegador por el usuario** — el Mongo self-hosted del EC2 no tiene puerto expuesto al host (ver `CLAUDE.md`), así que probar en local requiere el túnel SSH que el usuario ya usa normalmente; sin ambiente de staging, cualquier baja de prueba real marca un activo real como `'baja'` en producción (revertible borrando el registro desde la propia página).
- **Commit(s):** pendiente (sin commitear aún).

### 2026-08-25 — Plantilla Word: Formato de Salida por Venta de Activo (baja + venta)
- **Qué pasó:** el usuario pidió un formato en Word para dar de baja del inventario un activo que se vende, del mismo estilo visual que el "Formato de Salida de Equipos" (envíos, `shipmentPdf.js`), pero para venta en vez de traslado entre sucursales.
- **Qué se creó:** `generar_formato_salida_venta.py` (raíz del repo, mismo patrón que `generar_manual.py` con `python-docx`, branding naranja `#E8431A`/negro) genera `responsiva_ref/Formato_Salida_Venta_Activo.docx` — plantilla en blanco para imprimir/llenar a mano, **no** un flujo integrado al sistema (no hay ruta ni modelo nuevos, es documento suelto). Contenido: folio/fecha, datos del activo (tipo, marca/modelo, serie, no. de inventario, sucursal, condición, checkbox de formateo/borrado de datos corporativos), motivo de baja (venta), datos del comprador (empleado interno o externo/tercero, con campos de identificación para el caso externo), datos de la venta (monto, forma de pago, fecha, referencia), cláusula de "se vende tal cual, sin garantía" y 3 firmas: Entrega (IT), Autoriza (Gerente de Sistemas) y Compra (comprador).
- **Decisiones tomadas con el usuario:** autoriza el Gerente de Sistemas (mismo patrón que envíos, no Finanzas/Gerencia General); el comprador puede ser empleado interno o externo (ambos casos cubiertos); incluye monto+forma de pago, condición del equipo, y la cláusula de "tal cual sin garantía".
- **Pendiente si se quiere ir más allá:** convertirlo en un flujo real dentro del sistema (modelo `AssetSale`/reutilizar `status: 'baja'` de `Asset`, ruta que genere el PDF con pdfkit auto-llenado como los demás formatos) — por ahora es solo la plantilla en Word, a petición explícita del usuario.
- **Fix inmediato (mismo día):** la primera versión fijaba el ancho de las tablas celda por celda (`cell.width` en cada fila) en vez de por columna completa — el `tblGrid` interno del `.docx` quedaba desactualizado y Word recortaba el borde izquierdo de las etiquetas largas ("Tipo de activo:" se veía como "activo:"). Corregido en `set_col_widths()` usando `table.columns[i].width` (actualiza `tblGrid` y todas las celdas de la columna a la vez) + `w:tblLayout type="fixed"` explícito.
- **Commit(s):** pendiente (sin commitear aún).
...
### 2026-08-19 — FEATURE: Sucursal en Calendario ahora es multi-selección del catálogo real
- **Qué pasó:** el usuario pidió poder elegir una o varias sucursales del catálogo ya existente (el mismo que usa Empleados), en vez de escribirlas a mano.
...
### 2026-08-19 — FEATURE: Reporte semanal del becario, dentro del Calendario
- **Qué pasó:** el usuario pidió un formato de reporte semanal para que el becario (Atsiel) documente su actividad (soporte, limpiezas, mantenimientos, movimientos) — con medición real de SLA y calificaciones, dando pie a evaluar desempeño (mes de prueba). Primero se armó como Word estático; el usuario pidió meterlo al Calendario para que Atsiel lo abra/llene ahí, Miguel lo valide, y Lilly (y el resto) solo lo consulten. Dos dudas resueltas en el camino: "Elabora" y "Nombre del becario" eran lo mismo (se quitó la duplicidad en el Word), y la tabla de indicadores no se llena a mano — se calcula sola.
- **Qué cambié:** `backend/src/models/CalendarActivity.js` — nuevo `reportType:'becario_semanal'` + subdocumento `report` (resumen, otras actividades, cursos, autoevaluación, evaluación del supervisor, estado pendiente/llenado/validado) + `reportHistory` (copia de cada semana ya validada ANTES de resetear `report`, a propósito — a diferencia del resto de recurrentes, aquí si se necesita ver varias semanas seguidas para juzgar mejora real de desempeño). `backend/src/routes/calendarActivities.js` — `GET/PUT /:id/report` (el becario asignado guarda/envía su parte) y `PUT /:id/report/validate` (solo Miguel García, mismo criterio de "una sola persona" que `isVentasUser` en tickets.js; validar = completar, dispara el re-agendado semanal + push a Miguel al enviar y al becario al validar). Los indicadores y la lista de tickets atendidos se calculan en `computeReportMetrics()` a partir de tickets reales asignados al becario esa semana (SLA cumplido, calificación promedio, tickets ≤3, escalados, sin calificar) — nada se llena a mano. Componente nuevo `frontend/src/components/ReporteSemanalModal.jsx` con 3 modos: becario edita, Miguel valida, todos los demás solo consultan (incluye historial de semanas ya validadas).
- **Verificación:** `node -c` y `npm run build` sin errores.
- **Commit(s):** `489be3f`
- **Fix inmediato tras crear la actividad real:** `POST`/`PUT /:id` de `calendar-activities` no aceptaban `reportType` del body — la actividad recurrente real ("Reporte semanal de tickets...", asignada a Atsiel, viernes 21-ago) se creó con `reportType:'ninguno'` en vez de `'becario_semanal'`. Corregido en ambas rutas, desplegado, y el documento ya creado se corrigió con el mismo `PUT` una vez arreglado. Commit `717d9cd`.
...
- **Qué cambié:**
  - `frontend/src/components/TicketChatPanel.jsx` (nuevo) — chat extraído de `TicketDetailModal.jsx` (mismo criterio que `InternalNotesPanel.jsx`, 2026-07-24) para reutilizarlo en `BiRequestDetailModal.jsx` sin duplicar ~150 líneas ni arriesgar el modal general de Tickets, que no se tocó.
  - `frontend/src/pages/TicketsChats.jsx` — ya no lista Proyectos en la bandeja de conversaciones. `frontend/src/pages/TicketDetailModal.jsx` — si se abre un Proyecto desde el Tablero, muestra un aviso que manda al Kanban en vez de duplicar el chat ahí también.
  - `backend/src/routes/tickets.js` — nuevo `adminPortalUrlForTicket()`: el push al asignado cuando el empleado responde manda a `/bi/projects?ticket=` (BiLayout.jsx ya soportaba ese deep-link) en vez de `/tickets/general` para un Proyecto.
  - `frontend/src/components/BiRequestDetailModal.jsx` — recuadro que explica literalmente Chat/Notas públicas/Notas privadas; "Comentarios" (`projectComments`) renombrado a "Notas privadas"; nueva sección "Notas públicas" (reutiliza `InternalNotesPanel.jsx kind="public"`, ya existente).
  - `frontend/src/components/BiSolicitudDetailModal.jsx` — ahora sí renderiza `publicNotes` del lado del empleado (antes nunca las mostraba, aunque el backend ya las mandaba).
  - `frontend/src/components/InternalNotesPanel.jsx` — su `canManage` no conocía `isBiOnlyUser()`: sin el fix, cualquier persona de BI (no admin) viendo un Proyecto asignado a un compañero se topaba con `canManage:false` en el frontend, aunque el backend sí aceptara la nota de cualquier persona de BI.
  - **Fix de seguridad real encontrado en el camino:** `stripInternal()` nunca excluía `projectComments` — viajaban sin filtrar al navegador del empleado (nunca renderizados, pero visibles con cualquier herramienta de red). Ahora se excluyen igual que `internalNotes`.
  - `backend/src/routes/tickets.js`, `PUT /:id/bi-stage` — al marcar un Proyecto "Entregado" (ya ponía `status: 'resuelto'` desde ayer, habilitando el CSAT) ahora también manda un push al empleado avisándole que califique — antes no le llegaba ningún aviso.
- **Verificación:** `node -c`/`npm run build` sin errores.
- **Commit(s):** `9d3d959`

### 2026-08-18 — FIX: push notifications nunca llegan a usuarios atorados en un service worker viejo
- **Qué pasó:** el usuario reportó "muchos usuarios tienen prendidas las notificaciones push y tienen activos los permisos en el navegador y aún así NO les llegan las notificaciones".
- **Diagnóstico:** se confirmó con un envío real de prueba contra las 3 suscripciones de un admin real (Apple/FCM/Windows) que el backend manda bien — los 3 servicios de push responden 201 (aceptado para entrega). El problema es 100% del lado del navegador: el evento `push` solo lo recibe el service worker ACTIVO de cada quien, y sin `skipWaiting`, un service worker nuevo (con el código de `push-sw.js` correcto) se queda instalado y esperando indefinidamente mientras el viejo sigue activo — a menos que la persona note el aviso de "Actualizar" y le dé clic, y como ya se confirmó hoy mismo con el fix anterior, ese botón ni siquiera garantizaba funcionar. Cualquiera que se haya suscrito a push antes de un ciclo de actualización exitoso queda con una suscripción real y válida, pero un service worker activo sin el listener de `push` — silencioso, sin ningún error visible.
- **Qué cambié:** `frontend/vite.config.js` — se agrega `skipWaiting: true` a la config de workbox. Cualquier versión nueva del service worker se activa sola en segundo plano en cuanto el navegador la detecta, sin esperar ningún clic — el manejo de push/background queda siempre al día, independiente de si la persona nota o no el aviso de "Actualizar" (ese aviso se queda solo para refrescar el contenido VISIBLE de la página).
- **Autorreparable:** como `registration.update()` ya se llama periódicamente desde hace tiempo (mount/visibilitychange/intervalo, ver `UpdateToast.jsx`), esto se resuelve solo para TODOS los usuarios ya atorados en cuanto su navegador baje este `sw.js` nuevo — sin depender de ninguna versión previa del código del cliente ni de que nadie haga clic en nada.
- **Verificación:** `npm run build` sin errores; confirmado que `dist/sw.js` trae `self.skipWaiting()` de verdad.
- **Commit(s):** `ad53b82`
...
- **Qué cambié (fix 1):** `frontend/src/pages/MisTickets.jsx` — se exporta `CsatSurvey` (antes privado) para reutilizarlo. `frontend/src/components/BiSolicitudDetailModal.jsx` — se agrega el mismo widget cuando el ticket está resuelto. `frontend/src/pages/MisSolicitudes.jsx` — `onUpdated` refresca el estatus en la lista al calificar, igual que ya hacían Cuentas/Envíos.
- **Causa raíz 2 (bug de código, no solo dato suelto):** `backend/src/routes/tickets.js` — `POST /:id/satisfaction` (el que cierra el ticket al calificar) nunca limpiaba `awaitingCloseAuthorization` — un ticket marcado candidato a cierre por abandono que el empleado termina contestando/calificando tarde se quedaba con esa bandera pegada. Se encontró 1 caso real (`TICK-CDEEE7`) y se corrigió a mano en producción, con respaldo fresco verificado antes y confirmación explícita del usuario.
- **Causa raíz 3 (uso, no bug de UI):** Auditoría mostró que ese mismo día 2 admins distintos (Miguel Garcia, Felipe Gomez) mandaron el recordatorio masivo 40 segundos aparte sin saber uno del otro (y antes, 2026-08-06, Lilly Arroyo 8 veces en 4 segundos) — cada envío duplica el push a cada empleado pendiente. Se agrega `GET /tickets/remind-pending-ratings/last` y un aviso en `TicketsCalificaciones.jsx` (confirmación extra si el último envío fue hace menos de 15 min, sin bloquear el reenvío cuando sí hace falta).
- **Verificación:** `node -c`/`npm run build` sin errores; confirmado contra datos reales de Vanessa Guzman y del ticket `TICK-CDEEE7` antes y después del fix puntual.
- **Commit(s):** `b97ea86`
...
- **Qué cambié:** `backend/src/utils/slaClassifier.js` (nuevo) — `applySlaCategory` (movido desde `tickets.js` para reutilizarse) + `classifyByText`: mismo criterio de palabras clave a mano que ya usa el buscador de Mesa de Ayuda, sin IA de por medio. Regla explícita, ajustada en vivo con el usuario sobre 2 casos reales: un accesorio que la computadora **no reconoce/detecta** (driver) es Software y Sistema Operativo, no Periféricos — Periféricos se reserva para daño físico real. `RESOURCE_ITEM_SLA` — mapeo directo para recursos redirigidos a Ticket (ahí el nombre del recurso sí es la categoría, sin ambigüedad). `tickets.js`/`resourceRequests.js` — se llama como complemento cuando el slaHint del formulario no clasificó nada.
- **Además:** se corrigió a mano `TICK-1CC1A2` (de "Periféricos" a "Software y Sistema Operativo" — el SAE, un programa, no conectaba con la impresora), con confirmación explícita del usuario.
- **Verificación:** `node -c` sin errores; probado contra los casos reales discutidos (OneDrive, micrófono/driver, SAE/impresora, redirigido de Impresora); deploy verificado en vivo (backend conectado, sitio responde 200).
- **Commit(s):** `7dee863`
