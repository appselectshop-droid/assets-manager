...
// (PUT /:id/bi-stage) y la conversación con quien reportó, reutilizando
// POST /:id/reply — el mismo endpoint que ya usa TicketDetailModal.jsx,
// sin adjuntos (no hacía falta para este caso).
      ...
      <div className={styles.modal} onClick={(e) => e.stopPropagation()} style={{ maxWidth: '720px' }}>
        ...
        <div className={styles.modalBody}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
              ...
              {!isDone && (
                <select className={styles.input} value={ticket.biStage || 'recibido'} onChange={handleStageChange} disabled={stageSaving} style={{ maxWidth: '220px' }}>
                  {BI_STAGE_ORDER.map((s) => <option key={s} value={s}>{BI_STAGE_CONFIG[s].label}</option>)}
                </select>
              )}
            </div>
          ...
          {ticket.biRequestKind === 'proyecto'
            ? <ProjectFields data={ticket.biProjectData} />
            : <DatabaseFields data={ticket.biDatabaseRequest} />}
