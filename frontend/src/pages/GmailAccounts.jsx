...
export default function GmailAccounts() {
  ...
  const openResponsivaModal = async (account) => {
    ...
    // alguien más la corrigió (correo, empleado) después de que se cargó
    // esta lista, aquí ya no se ve lo viejo.
    try {
      const { data: fresh } = await api.get(`/gmail-accounts/${account._id}`);
      account = fresh;
                ...
                {selectedEmployee ? (
                  <div className={styles.assignSelected}>
                    ...
                    <button
                      ...
                      onClick={() => { handleEmployeeChange(''); setEmpSearch(''); }}
                    >
                      Cambiar
                    </button>
                  </div>
                        ...
                        {filteredEmps.length === 0 ? (
                          ...
                          filteredEmps.map((emp) => (
                            <button
                              ...
                              onClick={() => { handleEmployeeChange(emp._id); setEmpSearch(''); }}
                            >
