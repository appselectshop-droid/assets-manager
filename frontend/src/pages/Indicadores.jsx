...
// Reutiliza el mismo módulo de estilos que Dashboard.jsx a propósito — esta
// página es la mitad "analítica" de lo que antes era un solo Dashboard
// gigante; se separó para que la navegación tenga un bloque de Indicadores
// claro (pedido de dirección), pero visualmente sigue siendo la misma familia
// de tarjetas/KPIs.
...
export default function Indicadores() {
  ...
  useEffect(() => {
    if (!isAdmin) { setOpsRaw({}); return; }
    api.get('/tickets', { params: { status: 'abierto,en_proceso' } })
      .then((res) => setOpsRaw({ tickets: res.data }))
      .catch(() => setOpsRaw({}));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  const derived = useMemo(() => {
    ...
    if (auditRaw && usersRaw) {
      ...
      // para que Indicadores solo muestre actividad de gente que existe hoy.
      ...
      /* Todo, todo separado por sucursal: se agrupa el score por persona bajo
         ...
         activo, solo se muestra ese grupo. */
    ...
    /* ── Detalle de Tickets (desglose por tipo + recientes) ─────
       KPI de servicio del área alimentado por Tickets, aunque el sistema de
       Tickets en sí esté documentado/fuera de alcance aparte. */
    let ticketsSummary = null;
    if (opsRaw && opsRaw.tickets) {
      const byTypeTickets = {};
      opsRaw.tickets.forEach((t) => { byTypeTickets[t.ticketType] = (byTypeTickets[t.ticketType] || 0) + 1; });
      const typeBreakdown = Object.entries(byTypeTickets)
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.header}>
        <div>
          <h1 className={styles.greeting}>Indicadores</h1>
          <p className={styles.date}>KPIs de servicio del área — inventario, actividad del equipo y tickets.</p>
        </div>
      </div>
