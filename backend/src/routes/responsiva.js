...
function getEmpresaConfig(businessName) {
  ...
  const key = Object.keys(EMPRESA_CONFIG).find(
    (k) => k.toUpperCase() === businessName.toUpperCase().trim()
  );
...
router.get('/:employeeId', auth, async (req, res) => {
  try {
    ...
    // ── FUNDAMENTO LEGAL ─────────────────────────────────────────────────────
    const legalText = 'La asignación se sustenta en la Ley Federal del Trabajo: Art. 110 Fracc. I (límites para descuentos salariales). Art. 132 Fracc. III (proporcionar útiles, instrumentos y materiales necesarios; no exigir indemnización por desgaste natural). Art. 134 Fracc. IV y VI (ejecutar el trabajo con cuidado; restituir materiales no usados y conservar en buen estado los instrumentos). Art. 135 Fracc. III y IX (no sustraer útiles de trabajo ni usarlos para objeto distinto).';
