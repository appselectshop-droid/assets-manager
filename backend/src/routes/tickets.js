...
router.delete('/:id/messages/:messageId', async (req, res) => {
  try {
    ...
    if (message.from !== 'admin') {
      return res.status(400).json({ message: 'Solo se pueden eliminar mensajes enviados por Sistemas' });
    }
    if (message.deleted) {
    ...
    await ticket.save();
    logAction(req.user, 'editar', 'ticket', ticket._id, ticket.subject, `Eliminó un mensaje del chat del ticket ${ticket.folio}`);
    res.json(ticket);
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
